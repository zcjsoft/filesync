...
### V1.1.0 (最新版本)
- **修复权限错误处理**：增强文件复制时的权限检查，解决`[Errno 13] Permission denied`错误
- **优化特殊字符支持**：完善对包含特殊字符（$、%、^、&、~、`、!、=、,等）文件名的处理
- **改进文件跳过机制**：添加`_should_skip_file`方法，在遇到权限问题时优雅地跳过文件
- **修复RENAME事件参数顺序**：修正文件重命名事件中的参数顺序错误
- **增强错误处理**：改进异常处理机制，提高系统稳定性

### V1.0.0
- 基础文件同步功能实现
- 支持CREATE、MODIFY、DELETE、RENAME事件
- 并发同步和增量同步支持
- 大文件传输进度显示
...
- **权限错误处理**：增强权限检查，优雅处理`Permission denied`错误
- **特殊字符支持**：完整支持包含特殊字符（$、%、^、&、~、`、!、=、,等）的文件名
- **文件跳过机制**：在遇到权限问题时自动跳过文件，确保同步连续性
...
6. **权限错误处理**：当遇到`[Errno 13] Permission denied`错误时，程序会自动跳过该文件并继续同步其他文件，确保同步连续性。
7. **特殊字符支持**：程序已优化对包含特殊字符（$、%、^、&、~、`、!、=、,等）文件名的处理，确保文件名正确同步。
8. **文件锁定检测**：程序会检测Windows文件锁定状态，避免因文件被其他程序占用导致的同步失败。
